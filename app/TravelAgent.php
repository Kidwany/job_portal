...
class TravelAgent extends Authenticatable
{
    ...
    {
        $this->notify(new TravelAgentResetPassword($token));
    }

    public function printTravelAgentImage($width = 0, $height = 0)
    {
        ...
        // $logo = (null!==($logo)) ? $logo : 'img.png';
        return \ImgUploader::print_image("travel_agent_logos/$logo", $width, $height, '/travel_agent_logos/img.png', $this->name);
    }
    ...
    {
        return FavouriteTravelAgent::where('travel_agent_slug', 'like', $this->slug)->count();
    }

    public function getFollowerIdsArray()
    {
        return FavouriteTravelAgent::where('travel_agent_slug', 'like', $this->slug)->pluck('user_id')->toArray();
    }
