...
<div class="form-body">
        ...
        <div class="col-md-6">
            <div class="form-group {!! APFrmErrHelp::hasError($errors, 'logo') !!}">
                <div class="fileinput fileinput-new" data-provides="fileinput">
                    ...
                    <div> <span class="btn default btn-file"> <span class="fileinput-new"> Select Travel Agent logo </span> <span class="fileinput-exists"> Change </span> {!! Form::file('logo', null, array('id'=>'logo')) !!} </span> <a href="javascript:;" class="btn red fileinput-exists" data-dismiss="fileinput"> Remove </a> </div>
                </div>
                {!! APFrmErrHelp::showErrors($errors, 'logo') !!} </div>
        </div>
        @if(isset($travel_agent))
        <div class="col-md-6">
            {{ ImgUploader::print_image("travel_agent_logos/$travel_agent->logo") }}        
        </div>    
        @endif  
    </div>
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'name') !!}"> {!! Form::label('name', 'Travel Agent Name', ['class' => 'bold']) !!}
        {!! Form::text('name', null, array('class'=>'form-control', 'id'=>'name', 'placeholder'=>'Travel Agent Name')) !!}
        {!! APFrmErrHelp::showErrors($errors, 'name') !!} </div>
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'email') !!}"> {!! Form::label('email', 'Travel Agent Email', ['class' => 'bold']) !!}
        {!! Form::text('email', null, array('class'=>'form-control', 'id'=>'email', 'placeholder'=>'Travel Agent Email')) !!}
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'ceo') !!}"> {!! Form::label('ceo', 'Travel Agent CEO', ['class' => 'bold']) !!}
        {!! Form::text('ceo', null, array('class'=>'form-control', 'id'=>'ceo', 'placeholder'=>'Travel Agent CEO')) !!}
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'description') !!}"> {!! Form::label('description', 'Travel Agent details', ['class' => 'bold']) !!}
        {!! Form::textarea('description', null, array('class'=>'form-control', 'id'=>'description', 'placeholder'=>'Travel Agent details')) !!}
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'country_id') !!}"> {!! Form::label('country_id', 'Country', ['class' => 'bold']) !!}                    
        {!! Form::select('country_id', ['' => 'Select Country']+$countries, old('country_id', (isset($travel_agent))? $travel_agent->country_id:$siteSetting->default_country_id), array('class'=>'form-control', 'id'=>'country_id')) !!}
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'travel_agent_package_id') !!}"> {!! Form::label('travel_agent_package_id', 'Package', ['class' => 'bold']) !!}  
        {!! Form::select('travel_agent_package_id', ['' => 'Select Package']+$packages, null, array('class'=>'form-control', 'id'=>'travel_agent_package_id')) !!}
        {!! APFrmErrHelp::showErrors($errors, 'travel_agent_package_id') !!} </div>

    @if(isset($travel_agent) && $travel_agent->package_id > 0)
    <div class="form-group">
        {!! Form::label('package', 'Package : ', ['class' => 'bold']) !!}         
        <strong>{{$travel_agent->getPackage('package_title')}}</strong>
    </div>
    <div class="form-group">
        {!! Form::label('package_Duration', 'Package Duration : ', ['class' => 'bold']) !!}
        <strong>{{$travel_agent->package_start_date->format('d M, Y')}}</strong> - <strong>{{$travel_agent->package_end_date->format('d M, Y')}}</strong>
    </div>
    <div class="form-group">
        {!! Form::label('package_quota', 'Availed quota : ', ['class' => 'bold']) !!}
        <strong>{{$travel_agent->availed_jobs_quota}}</strong> / <strong>{{$travel_agent->jobs_quota}}</strong>
    </div>
    <hr/>
    @endif
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'is_active') !!}">
        ...
        <div class="radio-list">
            ...
            $is_active_2 = '';
            if (old('is_active', ((isset($travel_agent)) ? $travel_agent->is_active : 1)) == 0) {
                $is_active_1 = '';
    ...
    <div class="form-group {!! APFrmErrHelp::hasError($errors, 'is_featured') !!}">
        ...
        <div class="radio-list">
            ...
            if (old('is_featured', ((isset($travel_agent)) ? $travel_agent->is_featured : 0)) == 1) {
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        filterDefaultStates(<?php echo old('state_id', (isset($travel_agent)) ? $travel_agent->state_id : 0); ?>);
    });
        ...
        if (country_id != '') {
            $.post("{{ route('filter.default.states.dropdown') }}", {country_id: country_id, state_id: state_id, _method: 'POST', _token: '{{ csrf_token() }}'})
                    .done(function (response) {
                        ...
                        filterDefaultCities(<?php echo old('city_id', (isset($travel_agent)) ? $travel_agent->city_id : 0); ?>);
                    });
        }
    }
