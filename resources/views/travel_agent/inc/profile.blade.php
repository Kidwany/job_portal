...
{!! Form::model($travel_agent, array('method' => 'put', 'route' => array('update.travel.agent.profile'), 'class' => 'form', 'files'=>true)) !!}
<h5>{{__('Account Information')}}</h5>
<div class="row">
<div class="col-md-4">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'email') !!}">
			<label>{{__('Email')}}</label>
			{!! Form::text('email', null, array('class'=>'form-control', 'id'=>'email', 'placeholder'=>__('Travel Agent Email'))) !!}
...
<h5>{{__('Travel Agent Information')}}</h5>
<div class="row">
    <div class="col-md-6">
        <div class="formrow">
			<label>{{__('Travel Agent Logo')}}</label>
			{{ ImgUploader::print_image("travel_agent_logos/$travel_agent->logo", 100, 100) }} </div>
    </div>
    <div class="col-md-6">
        <div class="formrow">
            <div id="thumbnail"></div>
            <label class="btn btn-elegant"> {{__('Select Travel Agent Logo')}}
...
<div class="row">
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'name') !!}">
			<label>{{__('Travel Agent Name')}}</label>
			{!! Form::text('name', null, array('class'=>'form-control', 'id'=>'name', 'placeholder'=>__('Travel Agent Name'))) !!}
    ...
    <div class="col-md-6">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'ceo') !!}">
			...
			{!! Form::text('ceo', null, array('class'=>'form-control', 'id'=>'ceo', 'placeholder'=>__('Travel Agent CEO'))) !!}
    ...
    <div class="col-md-12">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'description') !!}">
			...
			{!! Form::textarea('description', null, array('class'=>'form-control', 'id'=>'description', 'placeholder'=>__('Travel Agent details'))) !!}
    ...
    <div class="col-md-4">
        <div class="formrow {!! APFrmErrHelp::hasError($errors, 'country_id') !!}">
			...
			{!! Form::select('country_id', ['' => __('Select Country')]+$countries, old('country_id', (isset($travel_agent))? $travel_agent->country_id:$siteSetting->default_country_id), array('class'=>'form-control', 'id'=>'country_id')) !!}
	...
	if (old('is_subscribed', ((isset($travel_agent)) ? $travel_agent->is_subscribed : 1)) == 0) {
		$is_checked = '';
	}
	?>
    ...
    <div class="col-md-12">
        <div class="formrow">
            <button type="submit" class="btn btn-amber">{{__('Update Profile and Save')}} <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        filterLangStates(<?php echo old('state_id', (isset($travel_agent)) ? $travel_agent->state_id : 0); ?>);
        ...
        if (country_id != '') {
            $.post("{{ route('filter.lang.states.dropdown') }}", {country_id: country_id, state_id: state_id, _method: 'POST', _token: '{{ csrf_token() }}'})
                    .done(function (response) {
                        ...
                        filterLangCities(<?php echo old('city_id', (isset($travel_agent)) ? $travel_agent->city_id : 0); ?>);
                    });
        }
    }
