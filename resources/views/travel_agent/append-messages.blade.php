...
<figure class="profile-picture"><?php if($msg->type=='message'){?> {{$seeker->printUserImage(100, 100)}} <?php }else{?>{{$travel_agent->printTravelAgentImage()}} <?php }?></figure>
...
if ($(".form-inline").length > 0) {
        $(".form-inline").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "{{route('travel.agent.submit-message')}}",
                    type: "POST",
